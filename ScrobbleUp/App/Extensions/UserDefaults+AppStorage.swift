...
extension UserDefaults {
    ...
    func observe<T: Equatable>(_ keyPath: KeyPath<Keys.Type, UserDefaultsKey<T>>, onChange: @escaping (T) -> Void) -> AnyCancellable {
    ...
    func observe<T: RawRepresentable>(_ keyPath: KeyPath<Keys.Type, UserDefaultsKey<T>>, onChange: @escaping (T) -> Void) -> AnyCancellable where T.RawValue: Equatable {
        let initialValue = get(keyPath)
        onChange(initialValue)
...
extension AppStorage {
    init(_ keyPath: KeyPath<Keys.Type, UserDefaultsKey<Value>>, store: UserDefaults = .standard) where Value == Bool {
        let key = Keys.self[keyPath: keyPath]
        self.init(wrappedValue: key.defaultValue, key.key, store: store)
    }
    
    init(_ keyPath: KeyPath<Keys.Type, UserDefaultsKey<Value>>, store: UserDefaults = .standard) where Value == Int {
        let key = Keys.self[keyPath: keyPath]
        self.init(wrappedValue: key.defaultValue, key.key, store: store)
    }
    
    init(_ keyPath: KeyPath<Keys.Type, UserDefaultsKey<Value>>, store: UserDefaults = .standard) where Value == String {
        let key = Keys.self[keyPath: keyPath]
        self.init(wrappedValue: key.defaultValue, key.key, store: store)
    }
    
    init(_ keyPath: KeyPath<Keys.Type, UserDefaultsKey<Value>>, store: UserDefaults = .standard) where Value: RawRepresentable, Value.RawValue == String {
        let key = Keys.self[keyPath: keyPath]
        self.init(wrappedValue: key.defaultValue, key.key, store: store)
    }
}
