    ...
    Form {
      Section("Player") {
        ...
        Picker("Open links in", selection: $openLinksWith) {
          Text("Current active player or last.fm").tag(OpenLinksWith.currentActivePlayerOrLastFm)
          Text("Current active player or Apple Music").tag(
            OpenLinksWith.currentActivePlayerOrAppleMusic)
          Text("Current active player or Spotify").tag(OpenLinksWith.currentActivePlayerOrSpotify)
          Divider()
