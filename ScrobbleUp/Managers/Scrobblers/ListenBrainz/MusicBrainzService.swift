...
actor MusicBrainzService {
	...
	func fetchRecordingReleases(mbid: String) async throws -> [[String: Any]]? {
		...
		guard
			let recordingURL = URL(
				string: "https://musicbrainz.org/ws/2/recording/\(mbid)?inc=releases&fmt=json")
		else {
			return nil
		}
	...
	func searchRelease(artist: String, album: String) async throws -> String? {
		...
		guard
			let url = URL(
				string: "https://musicbrainz.org/ws/2/release?query=\(query)&limit=1&fmt=json")
		else {
			return nil
		}
	...
	private func searchMusicBrainz(query: String) async throws -> String? {
		guard
			...
			let url = URL(
				string:
					"https://musicbrainz.org/ws/2/recording?query=\(encodedQuery)&limit=5&fmt=json")
		else {
		...
		do {
			let json = try await http.getJSON(url: url, headers: nil)
			guard let recordings = json["recordings"] as? [[String: Any]], !recordings.isEmpty
			else {
