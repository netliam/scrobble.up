...
final class ListenBrainzStatsService {
	...
	func fetchTopAlbums(period: TopAlbumPeriod, limit: Int = 9) async -> [ListenBrainzTopAlbum]? {
		...
		guard
			let url = URL(
				string:
					"\(config.baseURL)/1/stats/user/\(username)/release-groups?range=\(rangeParam)&count=\(limit)"
			)
		else {
			return nil
		}
	...
	private func fetchLovedTracksCount(username: String) async -> UInt? {
		guard
			let url = URL(
				string: "\(config.baseURL)/1/feedback/user/\(username)/get-feedback?score=1&count=0"
			)
		else {
			return nil
		}
