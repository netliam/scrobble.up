...
final class TopAlbumsUpdater {
    ...
    private func fetchLastFmAlbums(period: TopAlbumPeriod) async -> [AlbumData] {
        ...
        return topAlbums.map { album -> AlbumData in
            ...
            return AlbumData(
                ...
                playCount: playCount,
                action: { [weak self] in
                    self?.handleAlbumClick(artist: album.artist.name, album: album.name, service: .lastFm)
                }
            )
        }
    }
    ...
    private func fetchListenBrainzAlbums(period: TopAlbumPeriod) async -> [AlbumData] {
        guard let topAlbums = await listenBrainzManager.fetchTopAlbums(period: period, limit: 9) else {
            return []
        }
        ...
        return topAlbums.map { album -> AlbumData in
            ...
            return AlbumData(
                ...
                playCount: playCount,
                action: { [weak self] in
                    self?.handleAlbumClick(artist: album.artistName, album: album.releaseName, service: .listenBrainz, mbid: album.releaseGroupMbid)
                }
            )
        }
    }
    ...
    private func handleAlbumClick(artist: String, album: String, service: ScrobblerService, mbid: String? = nil) {
        Task {
            switch service {
            case .lastFm:
